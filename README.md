# kp-msx: серверный клиент для кинопаб для Media Station X.

# Прочитайте внимательно вот это вот: 

1. Этот проект не имеет никакого прямого отношения к кинопаб, к администрации кинопаб, к поддержке кинопаб, это просто еще один вариант сервировки. Администрация кинопаб и поддержка кинопаб не должны и не будут помогать вам с установкой и настройкой этой поделки. Не обращайтесь по этому проекту в поддержку кинопаб или в чаты кинопаб, вас будут называть обидными словами.
...
Это сервер, с которым общается телевизор, и клиент, общающийся с кинопаб. Типа прокси, но с преобразованием ответов в родной формат MSX. В данный момент основные блокировки затрагивают адреса самого кинопаб, но не видео. Если вынести общение с кинопаб туда, где он не заблокирован, и передавать переработанную информацию в MSX через заведомо рабочий канал (телевизор-сервер), то все должно работать.
...
В аргумент MSX вписать IP вашего сервера и порт (1234 в вашем случае) через двоеточие, например: `1.2.3.4:1234`.
...
Для того чтобы продукт заработал, нужны база данных и сервер, и их попробовать иногда дают бесплатно.
...
По нажатию Deploy Web Service либо получится все сразу, либо попросят карту. Если карту попросили -- значит, сработала фрод, можно попробовать зарегистрироваться с другого IP, в инкогнито, либо просто дать карту, с нее ничего списать не должны.
...
| PORT               | Порт, на котором будет запущен сервер, если запускать как `python api.py`, для uvicorn указывается отдельно. 10000 по умолчанию для render.com | `1234`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| PLAYER             | Плеер, который будет показывать кино. Для HLS нужен свой, для HTTP -- свой.                                                                    | [Стандартные](https://msx.benzac.de/wiki/index.php?title=Video/Audio_Plugin#:~:text=for%20more%20information.-,Examples,-%5Bedit%5D)<br/>HTTP: `http://msx.benzac.de/plugins/html5.html` <br/>HTTP с сабами: `http://msx.benzac.de/plugins/html5x.html` <br/> HLS: `http://msx.benzac.de/plugins/hls.html` <br/><br/> Допиленные мной <br/> HTTP с переделанными сабами: `https://slonopot.github.io/msx-html5xs/html5xs.html` <br/> HLS с дорожками и сабами (по умолчанию): `https://slonopot.github.io/msx-hlsx/hlsx.html` |
...
| QUALITY            | Качество, если протокол `http` или `hls`. Если не указано или не найдено, то максимально доступное                                             | `2160p` (4K должно быть включено в кинопаб) <br/> `1080p`, `720p`, `480p`                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
Если размещаете на render.com, можно создать несколько разных ссылок с разными параметрами, просто делайте новый Web Service справа сверху. Ссылку на монго можно использовать одну и ту же.

## Бонусы

По адресу `/subtitleShifter` есть утилита для синхронизации субтитров. Client ID есть либо в логе, либо в Кинопаб в устройствах. Content ID содержится в ссылке на видео. Утилита сделана с помощью Claude, поэтому за качество исполнения я не отвечаю, но оно работает.